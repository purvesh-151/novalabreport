...
if ( have_posts() ) {
	...
	while ( have_posts() ) {
		...
		BoldThemesFrameworkTemplate::$class_array = array( 'bt-article-list-item', 'animate', 'bt_bb_animation_fade_in', 'bt_bb_animation_move_up' );
		if ( BoldThemesFrameworkTemplate::$media_html == '' ) BoldThemesFrameworkTemplate::$class_array[] = 'bt-no-media';
		...
		<article class="bt-no-search-results bt_bb_section bt_bb_top_spacing_large bt_bb_bottom_spacing_large bt_bb_layout_boxed_1200">
			<div class="bt_bb_port">
				...
				echo boldthemes_get_heading_html(
					array(
						'headline' => esc_html__( 'We are sorry, no results for: ', 'novalab' ) . get_search_query(),
						'subheadline' => esc_html__( 'Back to homepage', 'novalab' ),
						'url' => home_url( '/' ),
						'size' => 'medium'
					)									 
				);
